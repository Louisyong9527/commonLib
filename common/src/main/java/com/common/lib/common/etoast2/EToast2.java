...
import com.common.lib.common.utils.OpPermissionUtils;
    ...
    private EToast2(Activity context, CharSequence text, int HIDE_DELAY, boolean withBg) {
        ...
        if (oldToast == null) {
            if (withBg){
                ...
                ((android.widget.Toast) toast).setGravity(Gravity.CENTER, 0, 0);// 位置居中
                ((android.widget.Toast) toast).setDuration(Toast.LENGTH_SHORT);
                ...
                ((android.widget.Toast) toast).setView(contentView);
            }else {
